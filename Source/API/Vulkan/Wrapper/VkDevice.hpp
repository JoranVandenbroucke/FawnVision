...
inline auto InitializeDevice(SVkDevice& device, const SVkInstance& instance, const SVkSurface& surface) noexcept -> int32_t
{
#ifdef _DEBUG
    ...
    {
        queueCreateInfos.emplace_back(VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO, nullptr, 0, static_cast<uint32_t>(device.queueFamily.graphicsFamily), static_cast<uint32_t>(queuePriorties[i].size()), queuePriorties[i].data());
    }
    ...
    if (const VkDeviceCreateInfo createInfo{
        ...
        !CheckVkResult(vkCreateDevice(device.deviceInfo.device, &createInfo, nullptr, &device.device)))
    {
        return -1;
    }
...
inline void Cleanup(const SVkDevice& device)
{
